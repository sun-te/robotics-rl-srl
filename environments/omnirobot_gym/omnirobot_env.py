...
class OmniRobotEnv(SRLGymEnv):
    ...
    def initVisualizeBoundary(self):
        ...
        if self.square_continual_move:
            ...
            pts = np.zeros(shape=[50, 2])
            pts[:, 0] = np.sin(t) * np.float32(RADIUS)
            pts[:, 1] = np.sin(t) * np.cos(t) * np.float32(RADIUS)

            self.eights = np.zeros_like(pts, dtype=int)
            #Transfer it into the real render windows
            for i in range(len(self.eights)):
                self.eights[i] = pos_transformer.phyPosGround2PixelPos(pts[i], return_distort_image_pos=False).squeeze()- (
                        np.array(ORIGIN_SIZE) - np.array(CROPPED_SIZE)) / 2.0
                self.eights[i][0] *= RENDER_WIDTH / CROPPED_SIZE[0]
                self.eights[i][1] *= RENDER_HEIGHT / CROPPED_SIZE[1]
                self.eights = self.eights.astype(np.int)
            self.eights = self.eights.reshape((-1, 1, 2))
    ...
    def visualizeBoundary(self):
        ...
        if self.square_continual_move:
            ...
            cv2.circle(self.observation_with_boundary, tuple(self.center_coordinates), np.float32(radius_converted),
                       (0, 150, 200), 2)
            cv2.polylines(self.observation_with_boundary, [self.eights], True, (0, 0, 200),2)
        #Add boundary of env
